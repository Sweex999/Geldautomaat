    ...
    public class User
    {
        ...
        public User(int AccountNumber)
        {
            if (this.DbConnection.IsConnect())
            {
                ...
                Command.CommandText = "SELECT * FROM `user` where `account_number` = @account_number";
            ...
            {
                MySqlCommand Command = new MySqlCommand("", this.DbConnection.Connection);
                Command.CommandText = "SELECT * from `user_transaction` WHERE `user_id` = @id ORDER by date desc limit 3";
        ...
        public void getWithdrawCount()
        {
            ...
            {
                MySqlCommand Command = new MySqlCommand("", this.DbConnection.Connection);
                Command.CommandText = "SELECT count(`id`) from `user_transaction` WHERE `user_id` = @id AND `date` LIKE @date AND `amount` LIKE '-%'";
                Command.Parameters.AddWithValue("@id", this.Id);
        ...
        public MySqlDataAdapter getAllTransaction()
        {
            ...
            if (this.DbConnection.IsConnect())
            {
                Adapter = new MySqlDataAdapter("SELECT `amount`, `date` from `user_transaction` WHERE `user_id` = @user_id  ORDER BY `date` DESC LIMIT 3", this.DbConnection.Connection);
        ...
        public void getBalance()
        {
            ...
            {
                MySqlCommand Command = new MySqlCommand("", this.DbConnection.Connection);
                Command.CommandText = "SELECT SUM(`amount`) from `user_transaction` WHERE `user_id` = @id";
                Command.Parameters.AddWithValue("@id", this.Id);
                MySqlDataReader Reader = Command.ExecuteReader();
                while (Reader.Read())
                {
